...
public class PostDAO extends BasicDAO<Posts, String> {
	...
	public List<Posts> SearchPost(String posttitle1) {
		...
		try {
			...
			/*
			 * DBCollection coll = db.getCollection("Posts"); List<Posts>
			 * postlist1 = new ArrayList<Posts>();
			 * 
			 ...
			 * 
			 * while(cursor.hasNext()){ postlist1.add( (Posts) cursor.next()); }
			 * 
			 */
			return query.asList();
			// return postlist1;
	...
	public double savepost(Posts post1) {
		...
		try {
			...
			MongoClient mongoClient = new MongoClient(MongoConnection.mongoServerIP, MongoConnection.mongoServerPort);
			...
			if (dbuname.equals(uname)) {
				...
				BasicDBObject newDocument = new BasicDBObject().append("$inc", new BasicDBObject().append("postid", 1));
	...
	/*
	 * public List<Posts> SearchPost(String postTitle) { List<Posts> post1 = new
	 * ArrayList<>();
	 * 
	 * try{ MongoClient mongoClient = new MongoClient( "10.197.24.69" , 27017 );
	 * 
	 * // Now connect to your databases
	 * 
	 * db = mongoClient.getDB(dbName);
	 * System.out.println("Connect to database successfully"); DBCollection coll
	 * = db.getCollection("Posts");
	 * 
	 * 
	 * Query<Posts> query =
	 * createQuery().field("title").contains(title).field("isbn").lessThan(1000)
	 * ; return query.asList();
	 * 
	 * 
	 * DBObject query = new BasicDBObject("posttitle", postTitle); // DBObject
	 * dbObject = coll.findOne(query); DBCursor dbcursor = coll.find(query);
	 * 
	 * 
	 * while (dbcursor.hasNext()){ // System.out.println(dbcursor.next());
	 * 
	 * //post1 = (List<Posts>) dbcursor.next(); String uname3 = (String)
	 * post3.get("uname"); String blog3 = (String) post3.get("postBlog");
	 * post1.setPostBlog(uname3); post1.set
	 * ///System.out.println("------------------------------");
	 * //System.out.println(blog3+"and "+ uname3); } }catch(Exception e){
