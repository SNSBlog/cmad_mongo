...
public class Controller {
	...
	// public Response find(User user4)
	{
		...
		// GenericEntity <List<BasicDBObject>>enty2 = new
		// GenericEntity<List<BasicDBObject>>(p1){};
	...
	public Response login(@FormParam("username") String login, @FormParam("password") String passwd) {
		try {
			...
			// GenericEntity<List<BasicDBObject>> enty1 = new
			// GenericEntity<List<BasicDBObject>> (lst1){};

			// Response.ok()
			return Response.ok(lst1.toString()).header(HttpHeaders.AUTHORIZATION, "Bearer" + token)
					.header("Access-Control-Allow-Origin", "*")
					.header("Access-Control-Expose-Headers", "POST,GET,PUT,DELETE")
	...
	private String issueToken(String login) {
		...
		try {
			String jwtToken = Jwts.builder().setSubject(login)
					...
					.setIssuedAt(new Date()).setExpiration(date1)
